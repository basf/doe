...
import pandas as pd
import numpy as np
from doe.design import get_formula_from_string, get_objective, constraints_as_scipy_constraints, n_ignore_eigvals
import opti
from formulaic import Formula
from pyreto.pymoo_algorithms.opti_to_pymoo_problem import opti_to_pymoo_problem
...
n_experiments = (
        len(model.terms) + 2 #- n_ignore_eigvals(problem, model_formula) 
    )
...
class MyProblem(pymooProblem):
    ...
    def __init__(self, problem: opti.Problem, model_type, n_experiments, **kwargs):
        ...
        super().__init__(n_var=ndim*n_experiments, n_obj=1, n_constr=n_experiments, xl=0.0, xu=1.0, **kwargs)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        X = self.model.get_model_matrix(pd.DataFrame(x, columns=self.problem.inputs.names)).to_numpy()
        
        X = X.reshape(popsize, self.n_experiments, len(self.model.terms))

        #compute objective
        F = -np.sum(np.log(np.linalg.eigvalsh(np.transpose(X,axes=[0,2,1]) @ X)[:,self.n_ignore_eigvals:]), axis=1)
