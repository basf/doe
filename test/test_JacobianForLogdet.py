import numpy as np
import pandas as pd
from doe.JacobianForLogdet import default_jacobian_building_block, JacobianForLogdet
import opti
...
def test_default_jacobian_building_block():
    ...
    model_terms = np.array(Formula("x1 + x2 + x3 + x1:x2 + x1:x3 + x2:x3 + {x1**2} + {x2**2} + {x3**2}").terms, dtype=str)
    x = [1,2,3]
    ...
    B = pd.DataFrame(B, columns=["1", "x1", "x2", "x3", "x1**2", "x2**2", "x3**2", "x1:x2", "x1:x3", "x2:x3"])
...
test_JacobianForLogdet_jacobian()


#TODO: test_jacobian --> vergleiche mit finite_differences, rechne explizit für dim=2 aus